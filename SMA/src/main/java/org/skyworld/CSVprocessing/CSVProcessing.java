package org.skyworld.CSVprocessing;
...
public class CSVProcessing {
    ...
    public static void main(String[] args) {
        String csvFile = "C:\\Users\\BKT\\Desktop\\PROJECTS\\JAVA\\Tasks\\SMA\\member_details.csv"; // Path file
        String excelFile = "finaltest4.xlsx"; // Excel file to be generated

        try {
            processCSV(csvFile, excelFile);
            System.out.println("CSV data exported to Excel successfully.");
            ...
            for (String[] row : csvData) {
                ...
                if (isValidId(row[0]) && isValidMobileNumber(row[2]) && isValidEmail(row[3])) {
                    if (gender != null && !gender.isEmpty()) {
                        ...
                        Sheet genderSheet = workbook.getSheet(filterGender);
                        if (genderSheet == null) {
                            genderSheet = workbook.createSheet(filterGender);
                        }
...
//            invalidWorkbook.close();
    ...
    public static boolean isValidJinsia(String jinsia) {
        // validation  for mobile numbers
        return jinsia != null && jinsia.isEmpty();
    }
